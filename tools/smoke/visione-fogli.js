...
global.document = {
    ...
    /* `querySelector`/`querySelectorAll`/`remove`: li usa `_openSummary`, il modale
       di riepilogo che CHIUDE `Pipeline.run`. Senza, l'ultimo passo lancia e il
       banco stampa un TypeError che sembra un guasto della pipeline. Il modale
       resta fuori dal perimetro: qui si prova che run arrivi in fondo. */
    createElement: () => Object.assign({ set textContent(v) { this.innerHTML = String(v); },
        querySelector: () => ({}), querySelectorAll: () => [], remove() { } }, nulla),
...
/* il budget di output: nell'app lo calcola app.js dal modello attivo (vuole il
   DOM del selettore) — qui basta l'identità, come nei test della suite. */
window.getMaxOutputTokens = (n) => n;
...
(async function () {
    ...
    /* solo i PDF: dal 6/9 (a8a8ba8) accanto a ognuno c'è il gemello `.html` in
       `Sorgenti/`, stesso stem — senza il filtro sarebbero quattro nomi. */
    const nomi2 = scritti.slice(primaScritti).filter(x => /Domande-aperte.*\.pdf$/.test(x));
