# DIARIO DI SVILUPPO — MappAI Swiss Edition (2026)

> **Che cos'è questo file.** Il diario giorno-per-giorno, potato da `CLAUDE.md` il
> **21 agosto 2026**. Le sue sezioni sono in ordine di SCRITTURA, non di verità:
> si legge per il **perché** di una decisione, mai per sapere com'è fatto il codice adesso.
> Per lo STATO vale **[`docs/HANDOFF.md`](HANDOFF.md)**; per gli invarianti e le trappole
> permanenti **[`GUIDA-ARCHITETTO.md`](../GUIDA-ARCHITETTO.md)**.
>
> ⚠️ Non entra più in contesto a ogni sessione: si consulta a domanda («perché il PDF è
> vettoriale?», «da dove nasce la regola dell'appiglio?»). Cercalo con `grep`, oppure
> `/graphify query "..."` che lo indicizza.
>
> Ordine: la **testata** raccoglie i riassunti giornalieri in ordine inverso (il più
> recente in cima); poi la §11 storica, con le sessioni in ordine di scrittura.

---

## TESTATA — riassunti giornalieri (dal più recente)
