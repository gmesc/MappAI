        ...
        window.refreshMagnifier = function() {
            ...
            const mapViewClone = document.getElementById('map-view').cloneNode(true);
            magnifierContent.innerHTML = '';
            magnifierContent.appendChild(mapViewClone);
        };
        ...
        window.readModalAloud = function (modalId) {
            ...
            if (text.trim()) {
                ...
                utterance.onstart = () => {
                    const btns = document.querySelectorAll('.tts-button');
                    btns.forEach(btn => {
        ...
        window.switchSidebarTab = function(tab) {
            const tabs = ['structure', 'notes', 'study', 'finder'];
            tabs.forEach(t => {
        ...
        window.renderTreeView = function() {
            ...
            if(rootNodes.length === 0) {
                container.innerHTML = '<p class="text-[10px] text-slate-400 italic">Nessuna struttura disponibile.</p>';
                return;
            }
        ...
        function updateModelCapabilities() {
            ...
            capsEl.innerHTML = `
                <div class="flex flex-wrap gap-1.5 mb-2">
                    ${kb.caps.map(c => `<span class="px-2 py-0.5 rounded-full text-[10px] font-bold ${
                        c === 'pdf' ? 'bg-red-100 text-red-700' :
        ...
        window.switchToMapLayout = function () {
            ...
            document.getElementById('sidebar-subtitle').innerText = `Progetto: ${appState.rootNodeLabel}`;
            ...
            const html = `
                <div class="animate-fade-in">
                    ...
                    <div class="pt-4 border-t border-slate-200 space-y-4">
                        ...
                        <div>
                            <h4 class="text-xs font-bold text-indigo-500 mb-2 uppercase tracking-wider flex items-center gap-1"><i data-lucide="bot" class="w-4 h-4"></i> Chiedi al Tutor AI</h4>
                            <div class="flex gap-2 items-center">
                                <input type="text" id="ai-sidebar-input" placeholder="Chiarimenti sul nodo..." class="flex-grow border border-slate-300 rounded-lg p-2.5 text-sm outline-none focus:ring-2 focus:ring-indigo-500" onkeypress="if(event.key === 'Enter') window.handleAITask()">
                                <button onclick="window.handleAITask()" class="bg-indigo-600 text-white p-2.5 rounded-lg shadow-md hover:bg-indigo-700 shrink-0"><i data-lucide="send" class="w-4 h-4"></i></button>
                            </div>
                        </div>
                    </div>
            </div>
        ...
        window.toggleDyslexicFont = function () {
            ...
            if (!isDyslexicFont) {
                ...
                const t = document.getElementById('a11y-panel-toggle');
                if(t) t.classList.remove('hidden');

                document.getElementById('font-track-landing').classList.add('active');
                document.getElementById('label-dyslexic-landing').classList.add('active-label');
                document.getElementById('label-space-mono-landing').classList.remove('active-label');

                document.getElementById('font-track-sidebar').classList.add('active');
                document.getElementById('label-dyslexic-sidebar').classList.add('active-label');
                document.getElementById('label-space-mono-sidebar').classList.remove('active-label');
            } else {
                document.body.classList.remove('font-dyslexic');
                document.documentElement.classList.remove('font-dyslexic');
                document.body.classList.remove('a11y-invert', 'a11y-low-contrast', 'a11y-high-contrast', 'a11y-grayscale');
                ...
                // Pulisci bottoni A11y Panel
                document.querySelectorAll('.a11y-btn').forEach(btn => btn.classList.remove('active'));
                
                document.getElementById('font-track-landing').classList.remove('active');
                document.getElementById('label-dyslexic-landing').classList.remove('active-label');
                document.getElementById('label-space-mono-landing').classList.add('active-label');

                document.getElementById('font-track-sidebar').classList.remove('active');
                document.getElementById('label-dyslexic-sidebar').classList.remove('active-label');
                document.getElementById('label-space-mono-sidebar').classList.add('active-label');
            }
        }
...
Suggerisci SOLO correlazioni semanticamente significative e plausibili. Max 10 correlazioni. Se non trovi correlazioni valide, rispondi con [].
Formato: [{"source":"id_a","target":"id_b","rel":"correlazione"}]`;
        ...
        window.updateUserNotesSidebar = function () {
            ...
            customNodes.forEach(n => {
                ...
                html += `<h4 class="font-bold text-xs text-indigo-700 flex items-center gap-1"><i data-lucide="tag" class="w-3 h-3"></i> ${n.label}</h4>`;
                if(n.hasCustomText) {
                    if(n.aiDesc && n.aiDesc !== n.desc) {
                        html += `<p class="text-[9px] text-slate-300 mt-1 line-clamp-2 italic border-l-2 border-slate-200 pl-2 mb-1">${n.aiDesc}</p>`;
                    }
                    html += `<p class="text-[10px] text-slate-800 mt-1 line-clamp-3 font-medium">${n.desc}</p>`;
                }
                if(n.image) {
                    html += `<img src="${n.image}" class="w-full h-16 object-cover rounded mt-2 border border-indigo-200">`;
                }
                html += `</div>`;
            });
...
Assicurati che "correct" indichi il numero (1, 2 o 3) della risposta corretta per ogni oggetto. Usa l'italiano.`;
        ...
        window.handleAITask = async function (taskType) {
            if (!currentNode) return;

            let customQuery = document.getElementById('ai-sidebar-input').value.trim();
            if (!customQuery) return;

            window.openAIModal('Analisi Fonti in corso...');

            let contextText = `NODO SELEZIONATO: ${cleanLabel(currentNode.label)}\n`;
            contextText += `DESCRIZIONE GENERALE: ${stripHTML(currentNode.desc || currentNode.content || "")}\n\n`;

            const inheritedNotes = window.getInheritedDatabase(currentNode.id);

            if (inheritedNotes.length > 0) {
                contextText += `DATABASE DELLE FONTI (Ereditate dai sottonodi):\n`;
                inheritedNotes.forEach(db => {
                    contextText += `- [${db.title} | ${db.source}]: ${stripHTML(db.text)}\n`;
                });
            }

            let prompt = `L'utente chiede: "${customQuery}"\n\nRispondi in modo didattico basandoti ESCLUSIVAMENTE sui concetti esposti nel seguente database:\n\n${contextText}`;

            try {
                const apiKey = window.getSystemKey();
                const payload = {
                    systemInstruction: { parts: [{ text: "Sei un Tutor per studenti. Rispondi in italiano usando solo formattazione HTML (<strong>, <p>, <ul>, <li>)." }] },
                ...
                document.getElementById('ai-modal-title').innerHTML = "Risposta AI Estesa";
                document.getElementById('ai-modal-body').innerHTML = resultHTML;
                document.getElementById('ai-sidebar-input').value = '';
            } catch (e) {
                document.getElementById('ai-modal-body').innerHTML = `<p class="text-red-500 font-bold">Impossibile processare la richiesta. Verifica l'API Key.</p>`;
            }
        }
        ...
        const StorageManager = {
            ...
            loadProject: function(id) {
                ...
                if(data) {
                    ...
                    if (!appState.extractionMode) appState.extractionMode = 'mindmap';
                    ...
                    if(appState.db && appState.db.links) {
                        appState.db.links.forEach(l => {
                            if (typeof l.source === 'object' && l.source !== null) l.source = l.source.id;
                            if (typeof l.target === 'object' && l.target !== null) l.target = l.target.id;
                        });
                    }
                    if(appState.db && appState.db.nodes) {
                        appState.db.nodes.forEach(n => {
                            delete n.vx; delete n.vy;
                        });
                    }
        ...
        const zooms = [1.0, 1.15, 1.30, 1.50, 1.75, 2.0, 3.0, 4.0, 5.0];
        
        window.cycleTextZoom = function() {
            currentZoomIdx = (currentZoomIdx + 1) % zooms.length;
            const z = zooms[currentZoomIdx];
            document.getElementById('btn-text-zoom').innerHTML = `<i data-lucide="zoom-in" class="w-3 h-3"></i> ZOOM TESTO x${(z === 1.0 ? '1.0' : z.toFixed(2))}`;
            const body = document.getElementById('source-modal-body');
            if (body) {
                body.style.fontSize = (z * 100) + '%';
            }
            window.safeCreateIcons();
        };
        ...
        window.resetA11yTools = function() {
            ...
            const btnZ = document.getElementById('btn-text-zoom');
            if(btnLh) btnLh.innerHTML = `<i data-lucide="move-vertical" class="w-3 h-3"></i> INTERLINEA x1.5`;
            if(btnZ) btnZ.innerHTML = `<i data-lucide="zoom-in" class="w-3 h-3"></i> ZOOM TESTO x1.0`;
            const body = document.getElementById('source-modal-body');
            if(body) {
        ...
        window.executeContextualAIExtension = async function() {
            ...
            try {
                ...
                const promptText = `Sei un costruttore di Knowledge Graph. 
Il nodo genitore selezionato dall'utente è:
ID: "${contextualAITargetNode.id}"
Label: "${contextualAITargetNode.label}"

L'utente ti ha fornito questo materiale per espandere il nodo:
"""
${sourceContent}
"""

Compito:
Genera unicamente nuovi sotto-nodi pertinenti a partire da questo materiale. 
Restituisci SOLO un JSON valido con questa struttura:
{
  "nodes": [
    {"id": "NODE_123", "label": "Nuovo Concetto", "content": "Spiegazione", "level": ${contextualAITargetNode.level + 1}}
  ],
  "links": [
    {"source": "${contextualAITargetNode.id}", "target": "NODE_123", "rel": "collegato_a"}
  ]
}

REGOLE CRITICHE:
1. NON INCLUDERE il nodo genitore originale nel JSON, lo abbiamo già.
2. TUTTI i nuovi nodi devono collegarsi (direttamente o indirettamente) al nodo genitore "${contextualAITargetNode.id}".
3. USA ID univoci e casuali per i nuovi nodi.
4. Restituisci SOLO il JSON puro, nessuna spiegazione esterna.`;
                ...
                newData.nodes.forEach(n => {
                    n.chunks = [sourceContent.substring(0, 500) + "... (Fonte Context AI)"];
                    n.x = contextualAITargetNode.x + (Math.random() * 100 - 50);
                    n.y = contextualAITargetNode.y + (Math.random() * 100 - 50);
