...
(function (root, factory) {
  ...
  function lexical(records, query, mode) {
    ...
    const phrases = A.frasiDaPagine(selected.map((r, i) => ({ n: i + 1, text: r.text })), { minSentenceWords: 1, maxSentenceChars: 500000 });
    const found = A.ancoraNodi([{ id: 'query', desc: query }], phrases, { maxPerNode: 20, relMin: 0 });
    const hits = found.perNodo?.query || [];
    return hits.map(hit => ({ ...selected[hit.page - 1], text: hit.text, scores: { lexical: hit.score } }));
  }
